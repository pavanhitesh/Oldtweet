...
Then(
  'User submits the created form and validates the response when IsRequiredStatus is {string} and value is {string}',
  async (isRequired: string, value: string) => {
    if (value !== 'No') {
      formPage.formSelect = `${await browser.sharedStore.get('formFieldTypes')}`;
      await formPage.selectByVisibleText(await formPage.formSelectDetails, value);
    }
...
After({ tags: '@TEST_PD-29756' }, async () => {
